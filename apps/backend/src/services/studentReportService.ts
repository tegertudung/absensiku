...
export function renderStudentReportPdf(
  report: Awaited<ReturnType<typeof buildStudentReport>>,
) {
  ...
  const commands: string[] = [
    ...
    text(
      50,
      815,
      report.settings.institutionName || report.settings.systemName,
      17,
      true,
      "1 1 1",
    ),
    ...
    for (const p of report.programs) {
      commands.push(
        text(
          50,
          y,
          `${p.type === "REGULAR" ? "Reguler" : "Privat"} - ${p.label}`,
          9,
        ),
    ...
    for (const s of report.privateSessions.slice(0, 15)) {
      ...
      commands.push(
        text(
          50,
          y,
          `${dateStr} - ${s.subjectName || "Tanpa Mapel"} (${s.tutorName})`,
          9,
          true,
        ),
        text(
  ...
  if (report.regularAttendance.length === 0) {
    ...
    for (const a of report.regularAttendance.slice(0, 12)) {
      const dateStr = new Intl.DateTimeFormat("id-ID", {
        dateStyle: "medium",
      }).format(new Date(a.sessionDate));
      commands.push(
        text(
          50,
          y,
          `${dateStr} - ${a.className || "-"} - ${a.subjectName || "-"}`,
          9,
        ),
        text(
          470,
