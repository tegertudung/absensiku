...
export async function createParent(data: {
  ...
  if (data.studentIds.length === 0) {
    throw new AppError(
      "Pilih minimal satu siswa (anak) untuk dihubungkan",
      400,
    );
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const parent = await tx.parent.create({
      data: {
        parentCode: await nextBusinessCode(tx, "parent"),
        userId: user.id,
        name: data.name,
        phone: data.phone,
      },
  ...
  const parent = await tx.parent.findUnique({
    ...
    include: {
      ...
      children: {
        include: {
          student: {
            select: { id: true, name: true, studentCode: true, status: true },
          },
        },
      },
    },
...
export async function resolveParentIdForUser(
  userId: string,
): Promise<string | null> {
...
export async function linkChild(
  ...
  if (existing)
    throw new AppError(
      "Siswa ini sudah terhubung ke akun orang tua tersebut",
      409,
    );
...
export async function unlinkChild(
  ...
  if (!link)
    throw new AppError("Relasi siswa-orang tua ini tidak ditemukan", 404);
