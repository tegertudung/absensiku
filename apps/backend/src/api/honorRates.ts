...
import {
  createHonorRate,
  listHonorRates,
  listHonorRateHistory,
  deactivateHonorRate,
} from "../services/honorRateService";
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const rate = await createHonorRate({
        ...parsed.data,
        effectiveFrom: parseBusinessDate(parsed.data.effectiveFrom),
      });
...
router.get(
  ...
  requireRole("ADMIN"),
  async (req: Request, res: Response) => {
    const sessionType = req.query.sessionType as
      "REGULAR" | "PRIVATE" | undefined;
    const programId =
      typeof req.query.programId === "string" ? req.query.programId : undefined;
    try {
      res.json({
        success: true,
        data: await listHonorRates(sessionType, programId),
      });
    } catch (err) {
      handleError(err, res);
    }
...
router.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      res.json({ success: true, data: await listHonorRateHistory() });
    } catch (err) {
      handleError(err, res);
    }
  },
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const rate = await deactivateHonorRate(
        req.params.id,
        req.user!.userId,
        parsed.data.reason,
      );
