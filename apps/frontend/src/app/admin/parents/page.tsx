...
interface ParentRow {
  ...
  children: Array<{ relationship: string | null; student: { id: string; name: string } }>;
}
...
export default function AdminParentsPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '', studentIds: [] as string[] });

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [parentsRes, studentsRes] = await Promise.all([api.get('/parents'), api.get('/students')]);
      setParents(parentsRes.data.data);
      setStudents(studentsRes.data.data.map((s: any) => ({ id: s.id, name: s.name })));
    } catch {
  ...
  async function handleCreate() {
    ...
    if (form.studentIds.length === 0) return setFormError('Pilih minimal satu siswa (anak).');
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Gagal membuat akun orang tua.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function toggleActive(parent: ParentRow) {
    ...
    try {
      ...
      setActionError(err.response?.data?.message || 'Gagal mengubah status akun.');
    } finally {
      setBusyId(null);
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title="Orang Tua"
        description="Kelola akun orang tua/wali dan siswa (anak) yang terhubung ke akun mereka."
        action={
          <button
            onClick={() => {
              resetForm();
            ...
            className="flex h-10 items-center gap-1.5 rounded-lg bg-navy-900 px-4 text-sm font-medium text-white hover:bg-navy-800"
          >
                ...
                {parents.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      {p.children.length === 0 ? (
                        <span className="text-xs text-gray-400">Belum ada anak terhubung</span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      >
                        {busyId === p.id ? 'Memproses...' : p.user.isActive ? 'Nonaktifkan' : 'Aktifkan'}
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && (
        <Modal title="Tambah Orang Tua" onClose={() => setShowForm(false)}>
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="parent-name" className="mb-1.5 block text-xs font-medium text-gray-700">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="parent-email" className="mb-1.5 block text-xs font-medium text-gray-700">
              ...
              <div>
                <label htmlFor="parent-password" className="mb-1.5 block text-xs font-medium text-gray-700">
            ...
            <div>
              <label htmlFor="parent-phone" className="mb-1.5 block text-xs font-medium text-gray-700">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-gray-700">
                Hubungkan ke Siswa (Anak) <span className="text-red-600">*</span>
              </label>
              <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border border-gray-300 p-2">
                {students.length === 0 ? (
                  <p className="px-2 py-2 text-xs text-gray-400">Belum ada data siswa.</p>
                ) : (
                  students.map((s) => (
                    <label
                      key={s.id}
