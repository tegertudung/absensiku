...
export default function AjukanPerubahanJadwalPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const current = res.data.data as ScheduleDetail;
      setSchedule(current);
      setForm((form) => ({
        ...form,
        date: current.startDate?.slice(0, 10) || form.date,
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        setConflicts(
          res.data.data.filter((c: ConflictRow) => c.scheduleId !== id),
        );
      } catch {
  ...
  async function handleSubmit() {
    ...
    if (form.reason.trim().length < 3)
      return setError("Alasan perubahan wajib diisi (minimal 3 karakter).");
    if (form.startTime >= form.endTime)
      return setError("Jam mulai harus sebelum jam selesai.");
    ...
    try {
      ...
      setError(
        err.response?.status === 409
          ? "Jadwal masih bertabrakan dengan sesi lain."
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4">
        ...
        {schedule.subject?.name && (
          <p className="mt-1 text-xs text-gray-500">{schedule.subject.name}</p>
        )}
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Pilih waktu pengganti untuk jadwal ini.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-[11px] text-gray-400">
            Jadwal rutin baru: setiap hari {DAY_NAMES[dayOfWeek]}
          </p>
        </div>
            ...
            <div>
              ...
              {conflicts.map((c) => (
                <p key={c.scheduleId} className="mt-0.5 text-xs text-amber-700">
                  Waktu ini tumpang tindih dengan &quot;{c.label}&quot; (
        ...
        <p className="text-center text-[11px] text-gray-400">
          Perubahan berlaku langsung — admin akan menerima catatan pengajuan
          ini.
        </p>
      </div>
    </div>
